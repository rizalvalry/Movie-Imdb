    ...
    <v-layout wrap>
      <v-flex xs12 mr-1 ml-1>
        <v-card>
          ...
          <v-card-actions>
            <v-btn flat color="warning" @click="back">back</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
      ...
      <v-flex xs12>
        <div class="text-xs-center">
          <v-dialog v-model="dialog" width="500" dark>
            <v-btn slot="activator" color="warning" dark>View Ratings</v-btn>
            <v-card>
              <v-card-title class="headline red lighten-2" primary-title>Ratings</v-card-title>
              <v-card-text>
                <v-simple-table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <tr v-for="(rating,index) in this.ratings" :key="index">
                      <td class="text-left">{{ratings[index].Source}}</td>
                      <td align="text-right">
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="warning" flat @click="dialog = false">OK</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        </div>
    ...
    movieApi
      ...
      .then((response) => {
        ...
        this.ratings.forEach(function (element) {
          ...
          element.Value =
            element.Value <= 12 ? element.Value / 2 : element.Value / 20
        })
        this.loading = false
      })
      .catch((error) => {
